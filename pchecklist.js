...
var passwordCheckOld = function()
{
    ...
    stringPassword = $("password").value;//receives password from user
    ...
    if (stringPassword.length < 8) //checks if password is less than 8 characters in length 
    {
    stringOutput = "Your password must be at least 8 characters long."; //if not greater than 8, error message displayed
    }
    else
    {
        for (var i = 0; i < arrayBadPasswords.length; i++) //length of password 
        {
            if (stringPassword === arrayBadPasswords[i])//password entered must exist within bad password array
            {
            stringOutput = "This is a bad password. Please try again.";//locates matching password and displays error message
            //break;
            }
        }
     
    }
    if (stringOutput === "") //if no error messages displayed
    {
        stringOutput = "Great password choice!"; //passes tests and displays congrats message
    }
    $("output").value = stringOutput; //passes length and bad password list   
};
